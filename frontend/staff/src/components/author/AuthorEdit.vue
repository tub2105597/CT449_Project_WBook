        ...
        <div class="page">
            ...
            <div class="px-5 py-3">
                <Form class="form" @submit="onSubmit" :validation-schema="authorFormSchema">
                    <legend class="text-center fw-bold">Chỉnh sửa thông tin tác giả</legend>
                    <div class="form-group mb-2">
                        <label for="tenTG" class="text-muted">Bút danh: </label>
...
const authorFormSchema = yup.object({
    tenTG: yup.string().required('Bút danh không được để trống')
});

async function onSubmit() {
    const result = await Swal.fire({
        title: 'Xác nhận',
        text: 'Bạn có chắc muốn cập nhật thông tin tác giả này không?',
        icon: 'warning',
    ...
    //Update author
    const response = await authorService.updateAuthor(route.params.id, author.value);

    if (response.status === 'success') {
        isUpdated.value = true;
        await Swal.fire({
            title: 'Thành công',
            text: 'Cập nhật thông tin tác giả thành công',
            icon: 'success',
            confirmButtonColor: '#198754'
        });
        isUpdated.value = true;
        ...
        await Swal.fire({
            title: 'Thất bại',
            text: 'Cập nhật thông tin tác giả thất bại',
            icon: 'error',
            confirmButtonColor: '#198754'
        });
    }
