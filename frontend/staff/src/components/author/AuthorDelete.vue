        ...
        <div class="page">
            ...
            <div class="px-5 py-3">
                <Form class="form" @submit="onSubmit">
                    <legend class="text-center fw-bold">Xóa tác giả</legend>
                    <div class="form-group d-flex mb-2">
                        <p class="text-muted fs-6 fw-bold">Tên tác giả: &nbsp;</p>
                        <p class="card-text ">{{ author.tenTG }}</p>
                    </div>
...
async function onSubmit() {
    const result = await Swal.fire({
        title: 'Xác nhận',
        text: "Bạn chắc chắn muốn xóa tác giả này chứ?",
        icon: 'warning',
    ...
    //Delete author
    const response = await authorService.deleteAuthor(route.params.id);

    await Swal.fire({
        title: 'Thành công',
        text: 'Xóa tác giả thành công',
        icon: 'success',
        confirmButtonColor: '#198754'
    });
