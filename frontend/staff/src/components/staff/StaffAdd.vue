        ...
        <div class="page">
            ...
            <div class="px-5 py-3">
                <Form class="form" @submit="onSubmit" :validation-schema="staffFormSchema">
                    <legend class="text-center fw-bold">Thông tin nhân viên</legend>
                    <div class="form-group mb-2">
                        <label for="email" class="text-muted">Mã NV: </label>
                        <input type="text" name="maNV" id="maNV" class="form-control" v-model="staff.maNV"/>
                    </div>
                    <div class="form-group mb-2" >
                    ...
                    <div class="form-group mb-3">
                        <button type="submit" class="btn btn-success update-btn float-end" @click.prevent="onSubmit">Thêm nhân viên</button>
                    </div>
                </Form>
            </div>
        </div>    
...
<script setup>
    ...
    async function onSubmit() {
        ...
        //Update staff
        const response = await staffService.createStaff(staff.value);
        ...
        if (response.status === 'success') {
            ...
            await Swal.fire({
                title: 'Thất bại',
                text: 'Thêm thông tin tài khoản thất bại',
                icon: 'error',
                confirmButtonColor: '#198754'
            });
        }
